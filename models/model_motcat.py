...
class MOTCAT_Surv(nn.Module):
    def __init__(self, fusion='concat', omic_sizes=[100, 200, 300, 400, 500, 600], n_classes=4,
        ...
        self.size_dict_WSI = {"small": [512, 256, 256], "big": [1024, 512, 384]}
        ...
        path_encoder_layer = nn.TransformerEncoderLayer(d_model=256, nhead=8, dim_feedforward=512, dropout=dropout, activation='relu',norm_first=True)
        ...
        omic_encoder_layer = nn.TransformerEncoderLayer(d_model=256, nhead=8, dim_feedforward=512, dropout=dropout, activation='relu', norm_first=True)
    ...
    def forward(self, **kwargs):
        ...
        x_rad = [kwargs['x_rad%d' % i] for i in range(1,7)]
        h_path_bag = self.wsi_net(x_path).unsqueeze(1) ### path embeddings are fed through a FC layer

        h_rad = [self.sig_networks[idx].forward(sig_feat) for idx, sig_feat in enumerate(x_rad)] ### each omic signature goes through it's own FC layer
        h_rad_bag = torch.stack(h_rad).unsqueeze(1) ### omic embeddings are stacked (to be used in co-attention)

        ### Coattn
        A_coattn, _ = self.coattn(h_path_bag, h_rad_bag)
        ...
        ### Rad
        h_omic_trans = self.omic_transformer(h_rad_bag)
        ...
        return logits, Y_hat, Y_prob, attention_scores