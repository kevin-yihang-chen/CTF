...
class MPMoE(nn.Module):
    ...
    def forward(self, image_features, rad):
        # image_features = self.image_encoder(image.type(self.dtype))
        prompts, tokenized_prompts, aux_loss = self.prompt_learner(rad, self.text)
...
class LPromptLearner(nn.Module):
    def __init__(self, clip_model, n_ctx, texts, n_experts=10):
        ...
        self.ctx_d = nn.Parameter(ctx_vectors[int(n_ctx/2):])
        self.n_ctx = n_ctx
        self.texts = texts
        self.n_experts = n_experts

        self.W_l = nn.Linear(512, 768)
    ...
    def forward(self, rad, texts=None):
        ctx_l = self.W_l(rad)
        ctx_d = self.ctx_d.reshape(self.n_experts,-1)
        gates, aux_loss = self.MoE(rad)
        ctx_d = torch.einsum("be, eh -> bh", gates, ctx_d)
        ctx_d = ctx_d.reshape(-1, 768)
        ctx_g = self.ctx_g
        ...
        ctx_l = ctx_l.unsqueeze(0).expand(n_cls, -1, -1)
        ctx_d = ctx_d.unsqueeze(0).expand(n_cls, -1, -1)
